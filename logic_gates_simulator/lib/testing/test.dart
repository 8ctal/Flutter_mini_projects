...
class _LogicGateSimulatorState extends State<LogicGateSimulator> {
  ...
  List<bool> dmux(bool input, bool sel) => [and(input, not(sel)), and(input, sel)];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Simulador de Compuertas Lógicas")),
      body: Column(
        children: [
          DropdownButton<String>(
            ...
            items: gateFunctions.keys.map<DropdownMenuItem<String>>((String gate) {
          ...
          Row(
            ...
            children: [
              _buildSwitch("A", inputA, (value) => setState(() => inputA = value)),
              if (selectedGate != "NOT")
                _buildSwitch("B", inputB, (value) => setState(() => inputB = value)),
              if (selectedGate == "MUX" || selectedGate == "DMUX")
                _buildSwitch("SEL", inputSel, (value) => setState(() => inputSel = value)),
            ],
          ),
          ...
          ElevatedButton(
            ...
            child: const Text("Ejecutar Circuito"),
          ),
          ...
          Expanded(
            child: Center(
              child: CustomPaint(
                ...
                painter: CircuitPainter(selectedGate, inputA, inputB, inputSel, signalPath, output),
              ),
            ),
          ),

          Text("Salida: ${output ? '1' : '0'}",
...
class CircuitPainter extends CustomPainter {
  ...
  CircuitPainter(this.gate, this.inputA, this.inputB, this.inputSel, this.signalPath, this.output);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (gate != "NOT") canvas.drawCircle(input2, 5, Paint()..color = Colors.blue);
    if (gate == "MUX" || gate == "DMUX") {
      ...
      if (signalPath[2] && inputSel) canvas.drawLine(selPos, gatePos, paintSignal);
    }
    ...
    if (signalPath[3] && output) canvas.drawLine(gatePos, outputPos, paintSignal);

    canvas.drawCircle(outputPos, 5, output ? Paint()..color = Colors.red : Paint()..color = Colors.black);
  }
